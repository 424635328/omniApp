...
- Screen: `app/src/main/java/com/example/energyflow/ui/chart/ChartScreen.kt` (~2200+ lines)
...
## 已知问题
- ChartScreen 使用 `collectAsState()` 而非 `collectAsStateWithLifecycle()`（MainScreen 已正确使用后者）
- ChartScreen 使用 `hiltViewModel()` 但 AppNavGraph 层已缓存 ViewModel，实际不会重复创建
...
| 常量 | 实际值 | 用途 |
|------|--------|------|
| `ElectricPeakColor` | #FF8800 (StaticPeakColor) | 峰电柱状图 |
| `ElectricValleyColor` | #8866DD (StaticValleyColor) | 谷电柱状图 |
| `NeonYellow` | #00A3FF (ElectricStart 别名) | KPI 强调色 |
| `OutlineDark` | #30364B | 平电柱状图 |
| `NeonBlue` | #00D4AA (WaterStart 别名) | AI分析按钮 |
| `SuccessGreen` | #00E676 | 费用/降序 |
| `WarningNeon` | #FFB020 | 阶梯预警 |
